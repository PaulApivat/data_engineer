...
query = "SELECT DISTINCT blockchain FROM user_raw_fact"
...
query = f"""
UPDATE user_raw_fact
SET blockchain_id = (SELECT blockchain_id FROM user_blockchain_dim WHERE user_blockchain_dim.blockchain = user_raw_fact.blockchain)
"""
conn.execute(query)
...
print(
    "Dimension Table 'user_blockchain_dim' created and linked to the Fact Table: user_raw_fact."
)
