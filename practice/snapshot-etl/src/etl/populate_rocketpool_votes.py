...
state = "all"
db_path = "rocketpool_space.db"  # SQLite database path
...
def fetch_and_insert_votes(cursor, proposal_id):
    ...
    for vote in votes:
        ...
        cursor.execute(
            """
            INSERT OR REPLACE INTO rocketpool_votes (id, voter, created, choice, space_id, proposal_id)
            VALUES (?, ?, ?, ?, ?, ?)
            """,
            (
                vote["id"],
...
print("----RocketPool ETL complete ----")
print("Data inserted into rocketpool_votes table successfully.")
